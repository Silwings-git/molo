    ...
    fn parse(input: ParseStream) -> syn::Result<Self> {
            ...
            match key.to_string().as_str() {
                ...
                other => {
                    return Err(syn::Error::new(
                        key.span(),
                        format!("unknown attribute {other}: only description / name / protected are supported"),
                    ));
                }
            }
            if !input.is_empty() {
        ...
        Ok(ToolArgs {
            description: description
                .ok_or_else(|| syn::Error::new(Span::call_site(), "missing required attribute description"))?,
            name,
            protected,
        })
    }
...
fn expand_tool(args: ToolArgs, item: ItemFn) -> syn::Result<proc_macro2::TokenStream> {
            ...
            FnArg::Typed(pat_type) => {
                ...
                if is_shared_state(&pat_type.ty) {
                    ...
                    let ident = match &*pat_type.pat {
                        ...
                        _ => {
                            return Err(syn::Error::new(pat_type.pat.span(), "parameters must be simple identifiers"));
                        }
                    };
    ...
    let sig_doc = format!("Generated by `#[molo::tool]`: tool implementation wrapping `{}`.", sig.ident);
