...
/// context via `protected_output`, so a single skill must not grow
/// unbounded.
...
/// the body then enters the conversation ledger as a protected message
/// ([`Tool::protected_output`](crate::Tool)),
/// exempt from window trimming, so skill instructions persist in long
/// conversations.
///
